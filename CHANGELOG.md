...
### Added
- 🧾 **Print status reported back to the web page** — a print job could fail for reasons the page had no way to see (printer powered off, cable unplugged, wrong TCP host in Settings): `window.freekiosk.print()` returned immediately and the failure was only visible in the native log. Every job now carries a `requestId` and produces a status object, readable two ways: the new `printAsync()` / `cutPaperAsync()` / `openCashDrawerAsync()` resolve to it (`{ ok: true, requestId }` or `{ ok: false, requestId, code, message }` with codes such as `INVALID_HOST`, `INVALID_PORT`, `INVALID_USB_IDS`, `PRINT_FAILED`, `BLOCKS_FAILED`), and a `window.freekiosk.onPrintResult` hook receives the status of **every** job — including ones started through the original fire-and-forget calls, so error reporting can be added to an existing page without touching its print calls. `print()` / `cutPaper()` / `openCashDrawer()` still return `undefined`, so pages written against the original API are unaffected. See [Direct Printing](docs/direct-printing.md).

- ⌨️ **`window.freekiosk.keyboard.show()` / `.hide()`** — raise or dismiss the Android soft keyboard from the kiosk page, for panels where the IME doesn't appear on its own or to get it out of the way after a form is submitted. Best-effort: Android only shows the IME for a focused editable element, so `show()` nudges `document.activeElement.focus()` first.

- ⬆️ **`window.freekiosk.getVersion()` / `checkUpdate({beta})` / `update({beta})`** — read the installed version (`versionName`, `versionCode`), query the GitHub release channel (stable or pre-release) and trigger the in-place self-update from the page, the same mechanism as Settings → About → Check for updates. `update()` downloads only when the release is genuinely newer, otherwise it resolves with `started: false`, so it is safe to call unconditionally.

- 📚 **[JavaScript API reference](docs/javascript-api.md)** — new documentation page covering the whole `window.freekiosk` namespace (printing, audio, keyboard, version/self-update, reboot) with a per-function detection cheat sheet, since older builds expose only part of the surface.

### Changed
- 🛠️ **`scripts/install.sh` now builds the release APK by default** — `android/app/release/` is gitignored, so a fresh clone had nothing to install and the script failed unless `--build` was passed. It now installs the npm dependencies when `node_modules` is missing, runs `./gradlew assembleRelease` (falling back to `sh ./gradlew` when a Windows clone lost the executable bit), and checks for `node` / `java` up front with a clear message. Pass `--no-build` to flash an APK already staged in `android/app/release/`; `--build` is still accepted as a no-op.
...
### Fixed
- 🖨️ **Intermittent "Permission Denied" when printing over USB** — Android grants USB device permission *per attachment*, so it was lost on every replug, reboot or re-enumeration (USB power-save, hub cycling), and `PrintTransport` then hard-failed the job. FreeKiosk now declares a `USB_DEVICE_ATTACHED` intent filter on `MainActivity` with a matching `usb_device_filter`, so ticking **"always open FreeKiosk for this device"** in the system dialog makes the grant persist across replug/reboot; and when permission is missing at send time the transport re-requests it instead of failing, with an error message that names the VID/PID and says how to make the grant stick.

- ⬛ **Blank strip at the bottom of the page after closing the soft keyboard** — some OEM WebViews leave a stale `ime()` bottom inset behind after the keyboard is dismissed, and `MainActivity` padded the content view by it unconditionally, hiding part of the page. The padding is now gated on `insets.isVisible(ime())`, so it collapses back to zero as soon as the keyboard closes.

- 📴 **OEM navigation bar hidden system-wide (including Android Settings), leaving no way back** on Rockchip RK3399 panels: the "Hide Navigation Bar (OEM panels)" lockdown removed the vendor bar globally and permanently. `deploy.bat` baked `qemu.hw.mainkeys=1` into `/system/build.prop` (root-only, irreversible from the app) AND the app's `SystemBarHelper` set persistent global flags without ever restoring them, so once you left FreeKiosk for Android Settings the bar stayed gone — no Back/Home/recents, no way to return or reboot without ADB. Now the OEM bar hiding is **scoped to FreeKiosk**: `MainActivity.onStop()` restores the bar when another app/screen (e.g. Settings) covers the kiosk, and it is re-hidden on return via `onWindowFocusChanged`. On RK3399 `rk3399_mid` firmware the only lever that actually removes the extended vendor bar (back/home/recents + volume) is the `com.elclcd.hidebar` / `com.elclcd.unhidebar` broadcasts its stock Settings app fires — these are now driven by the app, so the whole feature is controllable from **Settings → Security → Hide Navigation Bar (OEM panels)** with no `deploy.bat`/ADB needed. `deploy.bat` no longer writes `qemu.hw.mainkeys` and instead self-heals the legacy entry out of `build.prop` on already-provisioned rooted panels.
