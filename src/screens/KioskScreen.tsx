...
import { saveSecurePin, saveSecureMqttPassword, getSecureBasicAuthPassword, saveSecureBasicAuthPassword } from '../utils/secureStorage';
...
const KioskScreen: React.FC<KioskScreenProps> = ({ navigation }) => {
  ...
  const [directPrintEnabled, setDirectPrintEnabled] = useState<boolean>(false);
  ...
  const loadSettings = async (): Promise<void> => {
    try {
        ...
        if (pendingConfig) {
          ...
          for (const [key, value] of Object.entries(pendingConfig)) {
            if (typeof value === 'string') {
              ...
              } else if (key === '@basic_auth_password_pending') {
                // Website (HTTP Basic) auth password must be saved to Keychain
                await saveSecureBasicAuthPassword(value);
                console.log('[KioskScreen] Basic auth password saved to secure Keychain via pending ADB config');
              } else {
      ...
      const savedDirectPrintEnabled = bool(K.DIRECT_PRINT_ENABLED, false);
      setDirectPrintEnabled(savedDirectPrintEnabled);
        ...
        <>
          ...
          {dashboardModeEnabled && dashboardShowGrid ? (
            ...
            <WebViewComponent
              ...
              directPrintEnabled={directPrintEnabled}
              zoomLevel={zoomLevel}
              zoomMode={zoomMode}
